...
struct Meal: Identifiable, Hashable, Codable {
    // Ensures a unique .id for SwiftUI and Firestore integration
    var id: String { firestoreId ?? localId }
    @DocumentID var firestoreId: String?          // Firestore doc ID if available
    var localId: String = UUID().uuidString       // Unique UUID for arrays
    ...
    // Robust decoder (Firestore → Swift)
    init(from decoder: Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        firestoreId = try? container.decode(String.self, forKey: .firestoreId)
        localId = (try? container.decode(String.self, forKey: .localId)) ?? UUID().uuidString
        name = (try? container.decode(String.self, forKey: .name)) ?? "Untitled Recipe"
        description = (try? container.decode(String.self, forKey: .description)) ?? ""
        imageName = try? container.decode(String.self, forKey: .imageName)
        timeToCook = (try? container.decode(String.self, forKey: .timeToCook)) ?? "30 mins"
        ...
        // Difficulty: robust as string or int
        if let diffString = try? container.decode(String.self, forKey: .difficulty),
           let diff = Difficulty(rawValue: diffString) {
            difficulty = diff
        } else if let diffInt = try? container.decode(Int.self, forKey: .difficulty) {
            switch diffInt {
            case 1: difficulty = .easy
            case 2: difficulty = .novice
            case 3: difficulty = .intermediate
    ...
    // Robust encoder (Swift → Firestore)
    func encode(to encoder: Encoder) throws {
        var container = encoder.container(keyedBy: CodingKeys.self)
